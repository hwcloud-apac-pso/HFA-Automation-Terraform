...
### Sourth-West oubound : Transit Nat GW  - add snat rule for common cidr
resource "huaweicloud_nat_snat_rule" "egress_snat_rule_for_common_service" {
  provider = huaweicloud.transit_service
  nat_gateway_id = module.transit_service_nonprod_egress_vpc.egress_nat_gw_id
  floating_ip_id = module.transit_service_nonprod_egress_vpc.egress_nat_gw_eip
  source_type    = 1
